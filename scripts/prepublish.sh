...
# Claude-facing instruction files (the skill, its references, the build docs,
# the connector READMEs, docs/DEV.md) are the code carve-out; standalone/ is the labelled legacy setup.
set -u; cd "$(dirname "$0")/.."
bad=0
...
git ls-files '*.md' '*.html' | grep -vE '^(revenue-manager-plugin/skills/|revenue-manager-plugin/references/|build-|docs/DEV\.md|mcp-servers/|standalone/)' | xargs grep -l "—" 2>/dev/null && { echo "❌ em-dash in attendee-facing text"; bad=1; }
# The release zip must be exactly the committed tree, and every version string must agree.
# RELEASE_DIR overrides where the zips live (default ../release, next to this repo).
bash scripts/check-release.sh "${RELEASE_DIR:-../release}" || { echo "❌ release check failed (rebuild: bash scripts/build-release.sh)"; bad=1; }
