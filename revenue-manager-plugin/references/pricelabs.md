...
- **Never call the write tools directly.** `set_overrides`, `update_listings`, `delete_overrides` and any customization write exist, but every change, a rule change included, goes through the safe writer, `fetch/apply_change.py plan` then `apply` on a plain yes (`rollback` to undo). See SKILL.md Step 8. PriceLabs DSO and rule writes are live-tested (2026-09-25: a DSO and a day-of-week rule change on one listing, each applied, verified, undone and re-verified).
...
- **Occupancy health (framework KPI):** your `occupancy_next_*` vs `market_occupancy_next_*`. If market occupancy is materially higher than yours → listing-quality / visibility problem, not pricing (check ranking FIRST: RankBreeze or IntelliHost if present).
...
When you report ADR-vs-comp-median, be explicit which number you're using. Comp-set percentiles in neighborhood data include both listed and booked medians, so match like with like (ask-to-ask, cleared-to-cleared).
...
- **KPIs & benchmarks:** ADR vs comp median (neighborhood percentiles); Occupancy 70-85% peak / 40-60% off (`occupancy_next_*`); RevPAR = ADR × occ; pace vs STLY (Market KPI `Bookings STLY`); Review 4.8+ / below 4.6 = ranking problem; Response <1hr. Page-view/CTR/ranking KPIs (500-600 avg vs 2000-3500+ top) map to **RankBreeze or IntelliHost when present, else flag as a manual check**. PriceLabs does not expose ranking.
...
| `POST /v1/customizations/listing` | writer `rules_set` only | VERIFIED-LIVE 2026-09-25: The Urban Nest day-of-week Mon -8% -> -7%, re-read verified, rolled back by snapshot re-POST, re-read verified |
...
*PriceLabs is the tested primary pricing + comp engine for Revenue Manager v1 (reads live-tested; writes through the safe writer, not yet live-tested). Beyond is the supported alternative, built from Beyond's docs (`beyond.md`); Wheelhouse is detect-and-use; AirROI is an optional named-comp qualitative layer (returns native local currency via `currency=native`; verify it matches before mixing). AirDNA is not used.*
