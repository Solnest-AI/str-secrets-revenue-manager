...
> **Status: SHIPPED reference.** This file is hand-verified against the live Hospitable MCP tool surface and is the source of truth for how the revenue-manager skill reads/writes a Hospitable-connected operator. Every PMS the runner reads has its own reference next to this file; do **not** re-discover Hospitable's tool surface. Reads are live-tested; the calendar write (bottom of this file) was live-tested 2026-09-25: write, re-read verify and undo.
...
3. **`price.amount` is in the currency's minor unit (CENTS for USD, CAD, EUR): divide by 100.** A calendar night showing `price.amount = 28500` is **285.00** in the property's native currency. Never present cents as dollars. A currency without two decimals (JPY, KRW, VND have 0; KWD, BHD have 3) would read 100x low or 10x high, so the runner blocks a Hospitable property in one (`_money.py`) until it is converted end to end.
...
- **Parallel PMS pull (Agent 1):** `list_properties` → per-property `get_property_calendar` (365d forward) + `list_reservations` (all history) + `list_reviews` (≤100) + `list_transactions` (≥12mo). Parse `data.days[]`, **÷100 on `price.amount`** (two-decimal currencies only; others are blocked), carry native `currency`.
...
get_property_calendar.data.days[]:
    date                     → ISO date
    price.amount             → ASK nightly in minor units (CENTS for USD/CAD/EUR) → ÷100
...
**Remember:** calendar `price.amount` is the ASK in **minor units (cents)** and **equals** the PriceLabs pushed price; the calendar is ground truth; `user_price` in PriceLabs is stale; track ask (calendar) *and* cleared (ADR from reservations) separately; markup is what the operator states, per channel.
...
15000, KWD 150.25 -> 150250). The runner's read path keeps `price.amount` as hundredths, so
it now refuses any currency that is not two-decimal (`_mvp_pms._two_decimal_or_refuse`, shared
table in `_money.py`) instead of showing it 100x or 10x off. The writer does not share that
path.
