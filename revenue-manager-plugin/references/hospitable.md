...
> **Status: SHIPPED reference.** This file is hand-verified against the live Hospitable MCP tool surface and is the source of truth for how the revenue-manager skill reads/writes a Hospitable-connected operator. Every PMS the runner reads has its own reference next to this file; do **not** re-discover Hospitable's tool surface. Reads are live-tested; the calendar write (bottom of this file) is DOCS-ONLY until its first live write.
