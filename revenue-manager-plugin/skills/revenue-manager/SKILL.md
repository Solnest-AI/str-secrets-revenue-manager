...
- One file per PMS and pricing tool (`hospitable.md`, `guesty.md`, `ownerrez.md`, `hostaway.md`, `lodgify.md`, `uplisting.md`, `smoobu.md`, `hostfully.md`, `pricelabs.md`, `beyond.md` <!-- verify after feat/beyond merge -->): every endpoint the code calls, marked VERIFIED-LIVE or DOCS-ONLY, units, and the named gaps
...
| Beyond | `beyond_` or `mcp__beyond__` | Supported, built from Beyond's docs: runner `--pricing beyond` (a degraded card, every gap named) and writer `--target beyond` <!-- verify after feat/beyond merge --> |
...
| ... Beyond sets the prices | add `--pricing beyond` (PriceLabs is the default, `--pricing pricelabs`) <!-- verify after feat/beyond merge --> |
| ... the PMS sets the prices itself | add `--min-price "<property>=<amount>"` per property (no PMS API gives the writer a min) |
| Run one property | `fetch/analyze90.py --property "<exact name>"` (Beyond: add `--pricing beyond`) <!-- verify after feat/beyond merge --> |
