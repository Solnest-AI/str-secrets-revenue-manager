...
def bundle_roots(servers: dict, here: Path | None = None) -> list[Path]:
    """Where this bundle's root can live. Same lookup as _mvp_config.bundle_roots (config
    lane, 87d28bd); duplicated here until that lands, dedupe at merge.

    An installed plugin runs from ~/.claude/plugins/cache/..., where parents[4] is the cache,
    not the bundle, so it only counts when it really is a checkout. Otherwise the bundle is
    SKILL_PATH_REVENUE_MANAGER, from the environment or from the connections kit's .env (the
    kit found by walking up from each MCP server's cwd/command/args to fan-out-env.sh)."""
    ...
    for kit in dict.fromkeys(kits):
        found = _env_values(kit / ".env", {"SKILL_PATH_REVENUE_MANAGER"}).get(
            "SKILL_PATH_REVENUE_MANAGER")
        if found:
