...
def audit_statement(journal: dict) -> str:
    ...
    for op in env["operations"]:
        ...
        kind = prefix + kind
        # Only a removed override is "deleted". A listing-level null is a value: Beyond's max
        # restored to null means "no ceiling", and it is recorded as null.
        new = ("deleted" if op["after"] is None and op["kind"] == "override"
               else json.dumps(op["after"], sort_keys=True))
        rows.append("(" + ", ".join([
