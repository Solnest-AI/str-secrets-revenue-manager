...
class Cli(Base):
    ...
    def test_targets_from_other_branches_say_not_installed(self):
        for name, label in (("hostaway", "Hostaway"), ("lodgify", "Lodgify"), ("uplisting", "Uplisting"),
                            ("smoobu", "Smoobu"), ("hostfully", "Hostfully"), ("beyond", "Beyond")):
            with self.subTest(name=name):
                code, _, err = self.run_cli("plan", "--target", name, "--change", "missing.json")
                self.assertEqual(code, 2)
                self.assertIn(f"the {label} writer is not installed in this version", err)

    def test_registry_resolves_the_three_built_targets(self):
        for name, cls in (("hospitable", HospitableCalendarTarget), ("guesty", GuestyCalendarTarget),
                          ("ownerrez", OwnerRezCalendarTarget)):
            with self.subTest(name=name):
                self.assertIs(self._resolve(name), cls)
